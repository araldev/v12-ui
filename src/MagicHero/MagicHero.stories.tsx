...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story:
          'A scroll-driven hero section with GSAP ScrollTrigger. The hero (image + scroll hint) is visible from the start. As you scroll, the hero fades and scales down, a gradient overlay fades in, and a 3-line gradient text reveal appears at the end. Optionally, an SVG path can be provided to add a path-shaped mask reveal effect.',
      },
    },
  },
  component: MagicHero,
  argTypes: {
    ...
    svgPath: {
      type: 'string',
      description:
        'SVG path data. When provided, adds a path-shaped mask reveal effect.',
      control: 'text',
    },
    imageSrc: {
      type: 'string',
    ...
    maskColor: {
      type: 'string',
      control: 'color',
      description: 'Color of the SVG mask rect (dark fill).',
    },
...
export const NoSvgMask: Story = {
  name: 'Without SVG mask (just gradient)',
  args: {
    scrollDownText: 'Scroll down',
    quote1: 'Create',
    quote2: 'amazing',
    ...
    <MagicHero {...args}>
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 text-white pointer-events-none">
        ...
        <p className="text-lg opacity-80 text-center max-w-md">
          A subtitle that explains what your product does in one clear sentence.
        </p>
        <button
          type="button"
